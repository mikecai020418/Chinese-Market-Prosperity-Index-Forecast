...
def arima_forecast(df_column):
    ...
    # df_column.index = pd.to_datetime(df_column.index).to_period('Q').to_timestamp()
    model = pm.auto_arima(df_column, seasonal=False, trace=False)
...
def stl_ets_forecast(df_column):
    """Forecast using STL and ETS."""
    df_column1 = df_column.copy()
    
    # Backup original index
    original_index = df_column1.index
    # Convert to quarterly frequency for STL
    df_column1 = df_column1.asfreq('Q')
    stl = STL(df_column1, seasonal = 13)
    result = stl.fit()

    # Check if the trend component is not empty
    trend = result.trend.dropna()
    if len(trend) == 0:
        raise ValueError("Trend component is empty after STL decomposition. Cannot proceed with ETS forecasting.")
    ...
    forecast = model_fit.forecast(1)
    # Restore the original index (just in case you need it later in the code)
...
def prophet_forecast(df_column):
    ...
    # Slice the index to match the length of the data column
    df_prophet = pd.DataFrame({
        'ds': df.index[:len(df_column)],  # Align index length with the actual column data
        'y': df_column
    })
...
if __name__ == '__main__':
    ...
    # 提示：可以进一步添加更多前瞻性指标 - 及时调整和因子优化 - 若有时间：融合other_columns/ 加入修改注释
